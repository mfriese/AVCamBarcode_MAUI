...
namespace AVCamBarcode {
	...
	public partial class CameraViewController : UIViewController, IAVCaptureMetadataOutputObjectsDelegate, IItemSelectionViewControllerDelegate {
		...
		public CameraViewController (ObjCRuntime.NativeHandle handle) : base (handle) { }
		...
		private void ConfigureSession ()
		{
			if (setupResult == SessionSetupResult.Success) {
                ...
                var devices = AVCaptureDevice.Devices.Where(dd => dd.HasMediaType(AVMediaTypes.Video));

                AVCaptureDevice defaultVideoDevice = AVCaptureDevice.GetDefaultDevice (AVCaptureDeviceType.BuiltInWideAngleCamera, AVMediaTypes.Video, AVCaptureDevicePosition.Back) ??
													 AVCaptureDevice.GetDefaultDevice (AVCaptureDeviceType.BuiltInWideAngleCamera, AVMediaTypes.Video, AVCaptureDevicePosition.Front) ??
													 null;
				...
				if (this.session.CanAddOutput (metadataOutput)) {
					...
					DispatchQueue.MainQueue.DispatchAsync (() => {
						if (m_RegionOfInterestOverride != CGRect.Null)
                            // Restore the region of interest, if it's been setup
                            this.PreviewView.RegionOfInterestOverride = m_RegionOfInterestOverride;
		...
		partial void ChangeCamera (UIButton sender)
		{
			...
			DispatchQueue.MainQueue.DispatchAsync (() => {
				...
				this.ZoomSlider.MaxValue = (float) Math.Min (this.videoDeviceInput.Device.ActiveFormat.VideoMaxZoomFactor, 8);
		...
		private void OnRunningChanged (NSObservedChange change)
		{
			...
			DispatchQueue.MainQueue.DispatchAsync (() => {
				var test = AVCaptureDeviceDiscoverySession.Create(new AVCaptureDeviceType[] { AVCaptureDeviceType.BuiltInWideAngleCamera }, AVMediaTypes.Video, AVCaptureDevicePosition.Back);

				this.CameraButton.Enabled = true; //isSessionRunning && AVCaptureDevice.DevicesWithMediaType("Video").Length > 1;
			...
			foreach (var metadataObjectOverlayLayer in this.metadataObjectOverlayLayers) {
				...
				if (metadataObjectOverlayLayer.Path.ContainsPoint (location, false)) {
                    if (metadataObjectOverlayLayer.MetadataObject is AVMetadataMachineReadableCodeObject barcodeMetadataObject)
                    {
                        if (!string.IsNullOrEmpty(barcodeMetadataObject.StringValue))
                        {
							// Send Barcode to potential Listeners
							BarcodeSelected?.Invoke(this, barcodeMetadataObject.StringValue);
                        }
                    }
                }
			}
		...
		public event EventHandler<string> BarcodeSelected;
		...
		/// <summary>
		/// Enable setting the region of interest from outside the control so we
		/// can persist the last used state.
		/// </summary>
		private CGRect m_RegionOfInterestOverride = CGRect.Null;

        /// <summary>
        /// Can set this before launching the dialog to override initial setup.
        /// </summary>
        public void OverrideInitialRegionOfInterest(CGRect gRect)
			=> m_RegionOfInterestOverride = gRect;

		/// <summary>
		/// Can retrieve this after dialog finished to save last setting.
		/// </summary>
		public CGRect GetCurrentRegionOfInterest()
			=> PreviewView?.RegionOfInterest ?? CGRect.Null;
    }
}
